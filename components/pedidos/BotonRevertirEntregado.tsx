...
export default function BotonRevertirEntregado({ pedidoId, onSuccess, variant = "card" }: Props) {
  ...
  if (variant === "modal") {
      ...
      return (
        <div className="rounded-lg border border-gray-300 bg-gray-50 p-3">
          ...
          <div className="flex gap-2">
            <button
              onClick={confirmar}
              className="flex-1 rounded-lg bg-gray-700 px-4 py-2 text-sm font-semibold text-white hover:bg-gray-800 transition-colors"
            >
              Sí, revertir
            </button>
            <button
    ...
    return (
      <button
        onClick={pedir}
        disabled={fase === "cargando"}
        className="w-full rounded-lg border border-gray-200 bg-gray-50 px-4 py-2 text-sm font-medium text-gray-500 hover:bg-gray-100 transition-colors disabled:opacity-50"
      >
  ...
  if (fase === "confirmar") {
    return (
      <div className="mt-2 flex gap-1.5" onClick={(e) => e.stopPropagation()}>
        <button
          onClick={confirmar}
          className="flex-1 rounded-lg bg-gray-600 py-1.5 text-xs font-semibold text-white hover:bg-gray-700 transition-colors"
        >
          ¿Revertir envío? ↩
        </button>
        <button
  ...
  return (
    <button
      onClick={pedir}
      disabled={fase === "cargando"}
      className="mt-2 w-full rounded-lg border border-gray-200 bg-gray-50 py-1.5 text-xs font-medium text-gray-500 hover:bg-gray-100 transition-colors disabled:opacity-50"
    >
      {fase === "cargando" ? "Guardando…" : "↩ Marcar como no enviado"}
    </button>
  );
